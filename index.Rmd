...
I have already unzipped the data put the csv files in my ./data folder. Now I will load each of these csv files as data frames to my global environment.
...
The data frames named as daily* have date variables containing only date strings in %m/%d/%Y format and can easily be converted to POSIXct
``` {r, dates, cache = TRUE}
dailyActivity_merged$Date <- as.POSIXct(dailyActivity_merged$ActivityDate, 
                                                format = "%m/%d/%Y", 
                                                tz = "UTC")

dailyCalories_merged$Date <- as.POSIXct(dailyCalories_merged$ActivityDay, 
                                               format = "%m/%d/%Y", 
                                               tz = "UTC")

dailyIntensities_merged$Date <- as.POSIXct(dailyIntensities_merged$ActivityDay, 
                                                  format = "%m/%d/%Y", 
                                                  tz = "UTC")

dailySteps_merged$Date <- as.POSIXct(dailySteps_merged$ActivityDay, 
                                            format = "%m/%d/%Y", 
                                            tz = "UTC")
dailyActivity_merged <- subset(dailyActivity_merged, select = -ActivityDate)
dailyCalories_merged <- subset(dailyCalories_merged, select = -ActivityDay)
dailyIntensities_merged <- subset(dailyIntensities_merged, select = -ActivityDay)
dailySteps_merged <- subset(dailySteps_merged, select = -ActivityDay)
```

Also the datetime values need to be splitted and properly formatted in the other data frames too. That could be time consuming for all those 14 data frames, therefore I wrote a split_datetime_column function to find and format those values. I will use the function iteratively on the data frames that are not daily*. 


```{r, datetimes, cache = TRUE}
...
for (df_name in dataframes) {
  ...
  if ("Id" %in% colnames(df)) {
    ...
    cat(df_name, "has not 'Id'.\n")
  }
}
for (df_name in dataframes) {
  cat(df_name, "has", sapply(df_name, function(x) sum(is.na(x))), "nas", "\n")
}
```


```{r, join, cache = TRUE}
bellabeat_joined_all <- full_join(dailyActivity_merged, sleepDay_merged, by = c("Id", "Date"))

bellabeat_joined_no_nas <- inner_join(dailyActivity_merged, sleepDay_merged, by = c("Id", "Date"))

bellabeat_joined_only_nas <- anti_join(bellabeat_joined_all, bellabeat_joined_no_nas, by = c("Id", "Date"))

sapply(bellabeat_joined_no_nas, function(x) sum(is.na(x)))

```
``` {r, hourly}
hourly_dataframes <- dataframes[grepl("hourly", dataframes)]
...
weightLogInfo <- weightLogInfo_merged %>%
  select(Id, WeightKg) %>% group_by(Id) %>% summarise(mean_weight = mean(WeightKg))
```
